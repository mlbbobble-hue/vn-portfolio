...
with tab_wl:
    ...
    if wl.empty:
        ...
        for _, item in wl.iterrows():
            ...
            dist_str  = ""; dist_color = "#8C8C8C"
            if target > 0 and cur_price > 0:
                dp = (cur_price-target)/target*100
                dist_str = t("dist_to_target", pct=dp)
                dist_color = "#8A9A5B" if dp<=2 else "#D9C589" if dp<=10 else "#8C8C8C"
            ...
            cc = "#8A9A5B" if change>0 else "#C97A7E" if change<0 else "#8C8C8C"
            ...
            <div style='class="card" style="padding:14px 18px;margin:6px 0;">
                ...
                <div style='color:#A6A6A6;font-size:.82rem;margin-top:6px;'>{badge_str}{ey_str}</div>
            </div>""", unsafe_allow_html=True)
