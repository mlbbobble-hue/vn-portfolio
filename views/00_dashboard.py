...
.calendar-stock-tag {
    ...
    def render_grid_html(title_str, grid_data, events_data=None):
        events_map = events_data or {}
        html = []
    ...
    tab_june, tab_july = st.tabs([
        "📅 6月份 (本月)" if lang == "zh" else "📅 Tháng 06 (Hiện tại)",
        "🔥 7月份 (Q2財報預告)" if lang == "zh" else "🔥 Tháng 07 (Dự báo BCTC)"
    ])
    
    with tab_june:
        june_title = "📅 2026年6月份 - 越南股市結算期" if lang == "zh" else "📅 Tháng 06/2026 - Giai đoạn chốt số liệu"
        st.markdown(render_grid_html(june_title, grid_june, {}), unsafe_allow_html=True)
        
        tip_title = "💡 財報季小常識" if lang == "zh" else "💡 Kiến thức mùa báo cáo"
        tip_content = (
            "6 份月份為第二季 (Q2) 的最後一個月。越南上市公司的第二季財報通常會自 **7 月 15 日起至 7 月底**陸續密集公布。 "
            "因此，本月為財報數據產出的結帳空窗期。請點擊切換上方 **「🔥 7月份 (Q2財報預告)」** 標籤，查看 FPT、HPG 等核心持股的財報公布日程與成長預估！"
            if lang == "zh" else
                "Tháng 6 là tháng cuối cùng của Quý 2. Báo cáo tài chính Q2 (BCTC Q2) của các doanh nghiệp niêm yết thường sẽ công bố dồn dập từ **15/07 đến cuối tháng 07**. "
                "Do đó, tháng 6 là giai đoạn kết chuyển số liệu. Hãy chuyển sang tab **\"🔥 Tháng 07 (Dự báo BCTC)\"** để xem lịch dự báo và triển vọng tăng trưởng của FPT, HPG, MWG..."
        )
        st.markdown(f"""
        <div class="cathay-card" style="background: rgba(0, 240, 255, 0.03); border: 1px solid rgba(0, 240, 255, 0.2); border-radius: 12px; padding: 16px; margin-top: 16px;">
            <div style="font-weight: bold; color: #00F0FF; font-size: 14px; margin-bottom: 6px;">{tip_title}</div>
            <div style="font-size: 13px; color: #cbd5e1; line-height: 1.5;">{tip_content}</div>
        </div>
        """, unsafe_allow_html=True)
        
    with tab_july:
        july_title = "📅 2026年7月份 - 越南股市財報預告時間表" if lang == "zh" else "📅 Lịch công bố BCTC Q2 - Tháng 07/2026 (Dự báo)"
        st.markdown(render_grid_html(july_title, grid_july, events_july), unsafe_allow_html=True)
